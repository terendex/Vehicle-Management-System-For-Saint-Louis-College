"""What a fetcher registration carries — Data Privacy Office.
...
# The ID is still written into the JSON here on purpose: rows filed before
# this change carry one, and neither the review payload nor the emails may show it.
STUDENTS = [
...
class FetcherEmailTests(TestCase):
    ...
    def test_pending_email_does_not_print_a_student_id(self):
        """DPO — the row exists but the number is withheld."""
        send_pending_email(self.reg)
...
class WithheldFieldsAreAbsentFromEmailsTests(TestCase):
    ...
    Rows filed before this change still hold an address, a contact number and an
    ...
    def test_the_pdf_still_builds_for_a_legacy_row(self):
        """The rows the DPO dropped were part of every section it prints."""
